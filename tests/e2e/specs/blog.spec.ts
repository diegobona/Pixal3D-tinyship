...
test.describe('Blog', () => {
  ...
  // ── D) Pixal3D Static Guides ──────────────────────────────────

  test.describe('Pixal3D Static Guides', () => {
    const staticSlugs = [
      'pixal3d-model-uses',
      'image-to-3d-model',
    ...
    test('shows all six static guides with topic-specific raster covers in order', async ({ page }) => {
      const discoveredSlugs: string[] = [];

      for (let pageNumber = 1; pageNumber <= 50 && discoveredSlugs.length < staticSlugs.length; pageNumber += 1) {
        await page.goto(`${PAGES.blog}?page=${pageNumber}`, { timeout: TIMEOUTS.navigation });

        const hrefs = await page.locator('main a[href*="/blog/"]').evaluateAll((links) =>
          links.map((link) => link.getAttribute('href') || '')
        );
        const pageStaticSlugs = hrefs
          .map((href) => href.split('/blog/').at(-1) || '')
          .filter((slug) => staticSlugs.includes(slug) && !discoveredSlugs.includes(slug));

        for (const slug of pageStaticSlugs) {
          const card = page.locator(`main a[href$="/blog/${slug}"]`).first();
          await expect(card.locator('img')).toHaveAttribute('src', `/blog-covers/${slug}.webp`);
          discoveredSlugs.push(slug);
        }

        const hasNextLink = await page.locator(`a[href*="page=${pageNumber + 1}"]`).count();
        const nextButton = page.getByRole('button', { name: /next/i });
        const hasEnabledNextButton = await nextButton.count() > 0 && await nextButton.isEnabled();
        if (!hasNextLink && !hasEnabledNextButton) break;
      }

      expect(discoveredSlugs).toEqual(staticSlugs);
    });

    test('keeps the AnyPoses link and supplied figure together in use case one', async ({ page }) => {
      await page.goto(`${BASE}/blog/pixal3d-model-uses`, { timeout: TIMEOUTS.navigation });

      await expect(page.getByRole('heading', {
        level: 1,
        name: 'What Can You Do with a 3D Model Generated by Pixal3D?',
      })).toBeVisible();

      const firstUseHeading = page.getByRole('heading', {
        level: 2,
        name: '1. Use it as a prop when building a 3D scene',
      });
      const firstUseSection = page.locator('section').filter({ has: firstUseHeading });
      const anyPosesLink = firstUseSection.locator('a[href="https://anyposes.com"]');
      const suppliedFigure = firstUseSection.locator('xpath=following-sibling::*[1][self::figure]');

      await expect(anyPosesLink).toHaveAttribute('target', '_blank');
      await expect(anyPosesLink).toHaveAttribute('rel', 'noreferrer noopener');
      await expect(suppliedFigure.locator('img')).toHaveAttribute(
        'src',
        '/blog-covers/anyposes-custom-prop.png'
      );
      await expect(
        suppliedFigure.locator('xpath=following-sibling::*[1]//h2')
      ).toHaveText('2. Turn an idea into a fast concept prototype');
    });

    test('renders localized Chinese copy for the new guide', async ({ page }) => {
      await page.goto('/zh-CN/blog/pixal3d-model-uses', { timeout: TIMEOUTS.navigation });

      await expect(page.getByRole('heading', {
        level: 1,
        name: '用 Pixal3D 生成 3D 模型后，可以用来做什么？',
      })).toBeVisible();
      await expect(page.getByRole('heading', {
        level: 2,
        name: '1. 用作搭建 3D 场景时的道具',
      })).toBeVisible();
      await expect(page.getByText(
        '从 3D 场景道具、游戏原型到商品预览、AR 体验和 3D 打印准备，了解如何真正用好 Pixal3D 生成的模型。'
      )).toBeVisible();
      await expect(page.getByText(
        '生成模型后，最快上手的方式之一，就是把它作为道具放到角色或环境周围。'
      )).toBeVisible();
      await expect(page.locator('a[href="https://anyposes.com"]')).toBeVisible();
      const figure = page.locator('figure');
      await expect(figure.locator('img')).toHaveAttribute(
        'alt',
        'AnyPoses 3D 场景编辑器，画面中包含沙发、人体模型、花瓶和上传自定义道具按钮'
      );
      await expect(figure.locator('figcaption')).toHaveText(
        'AnyPoses 支持上传 GLB 或 STL 文件作为自定义场景道具。'
      );
    });
  });
});
